...
<div class="space-y-6 max-w-4xl">
    <div class="flex items-center space-x-3">
        <a href="<?php echo e(route('admin.categories.index')); ?>" class="w-8 h-8 flex items-center justify-center rounded-xl bg-slate-100 dark:bg-white/[0.05] text-slate-500 hover:text-slate-900 dark:hover:text-white transition-colors">
            <i class="fas fa-arrow-left text-sm"></i>
        </a>
        <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Edit Category: <?php echo e($category->name ?? 'Unnamed'); ?></h1>
    </div>

    <form action="<?php echo e(route('admin.categories.update', $category->id)); ?>" method="POST" enctype="multipart/form-data" class="glass-card rounded-2xl p-6 sm:p-8">
        <?php echo csrf_field(); ?>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="md:col-span-2">
                ...
                <input type="text" name="name" required value="<?php echo e(old('name', $category->name)); ?>" class="w-full bg-slate-50 dark:bg-[#0a0b1c]/50 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/50" placeholder="e.g. Standard Health Checkup">
                <?php $__errorArgs = ['name'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> <span class="text-xs text-rose-500 mt-1"><?php echo e($message); ?></span> <?php unset($message);
...
unset($__errorArgs, $__bag); ?>
            ...
            <div class="md:col-span-2">
                ...
                <div id="subCategoriesContainer" class="space-y-4">
                    ...
                    <?php $__currentLoopData = $subs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $sub): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="flex items-start space-x-3 sub-category-row bg-slate-50/50 dark:bg-white/[0.02] p-4 rounded-xl border border-slate-100 dark:border-white/[0.05]">
                        <div class="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" name="sub_category_name[<?php echo e($index); ?>]" value="<?php echo e($sub['name'] ?? ''); ?>" 
                            ...
                            <div>
                                ...
                                <?php if(!empty($sub['image'])): ?>
                                    <div class="mb-2 flex items-center space-x-3">
                                        <img src="<?php echo e(Storage::url($sub['image'])); ?>" alt="<?php echo e($sub['name'] ?? ''); ?>" class="w-10 h-10 rounded-lg object-cover border border-slate-200 dark:border-white/10">
                                        <span class="text-xs text-slate-500">Current Image</span>
                                    </div>
                                    <input type="hidden" name="sub_category_old_image[<?php echo e($index); ?>]" value="<?php echo e($sub['image']); ?>">
                                <?php endif; ?>
                                <input type="file" name="sub_category_image[<?php echo e($index); ?>]" accept="image/*" 
                        ...
                        <button type="button" class="remove-btn mt-5 w-10 h-10 flex-shrink-0 flex items-center justify-center rounded-xl bg-rose-50 text-rose-500 hover:bg-rose-100 dark:bg-rose-500/10 dark:text-rose-400 dark:hover:bg-rose-500/20 transition-colors <?php echo e(count($subs) === 1 ? 'hidden' : ''); ?>">
...
unset($__errorArgs, $__bag); ?>
        ...
        <div class="mt-8 flex justify-end space-x-3 pt-6 border-t border-slate-100 dark:border-white/[0.06]">
            <a href="<?php echo e(route('admin.categories.index')); ?>" class="px-5 py-2.5 rounded-xl text-sm font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-white/[0.05] transition-colors">
                Cancel
            </a>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        let subCatIndex = <?php echo e(max(array_keys($subs)) + 1); ?>;
...
<?php echo $__env->make('admin.layout.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\laravel\outsourcelab\resources\views/admin/categories/edit.blade.php ENDPATH**/ ?>