<?php $__env->startSection('title', 'Field Agents & Collectors'); ?>
<?php $__env->startSection('header', 'Field Agent / Sample Collector Management'); ?>
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <span class="font-bold text-slate-900 block"><?php echo e($agent->name); ?></span>
                                    <span class="text-[11px] text-slate-400">Joined <?php echo e($agent->created_at->format('M Y')); ?></span>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4">
                            <div class="text-xs space-y-0.5">
                                <a href="tel:<?php echo e($agent->phone); ?>" class="font-bold text-slate-800 hover:text-indigo-600 block">
                                    <i class="fas fa-phone-alt text-slate-400 mr-1 text-[10px]"></i> <?php echo e($agent->phone); ?>

                                </a>
                                <span class="text-slate-500 block">
                                    <i class="fas fa-envelope text-slate-400 mr-1 text-[10px]"></i> <?php echo e($agent->email); ?>
                        ...
                        <td class="px-6 py-4 text-xs">
                            <span class="font-semibold text-slate-700 block"><?php echo e($agent->city ?: 'N/A'); ?></span>
                            <?php if($agent->vehicle_number): ?>
                                <span class="text-slate-400 flex items-center gap-1 mt-0.5">
                                    <i class="fas fa-motorcycle text-[10px]"></i> <?php echo e($agent->vehicle_number); ?>
                        ...
                        <!-- Tasks Count -->
                        <td class="px-6 py-4 text-xs">
                            <a href="<?php echo e(route('admin.bookings.index', ['agent_id' => $agent->id])); ?>" class="inline-flex items-center gap-1 font-bold text-indigo-600 hover:text-indigo-800 bg-indigo-50 hover:bg-indigo-100 px-2.5 py-1 rounded-lg transition" title="View assigned bookings for this agent">
                                <i class="fas fa-clipboard-check text-indigo-500"></i>
                                <span><?php echo e($agent->active_bookings_count ?? 0); ?> Active</span>
                        ...
                        <!-- Status -->
                        <td class="px-6 py-4">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold <?php echo e($agent->status === 'active' ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-600'); ?>">
                                <span class="w-1.5 h-1.5 rounded-full <?php echo e($agent->status === 'active' ? 'bg-emerald-500' : 'bg-slate-400'); ?> mr-1.5"></span>
                        ...
                        <td class="px-6 py-4 text-right space-x-1.5">
                            ...
                            <a href="<?php echo e(route('admin.bookings.index', ['agent_id' => $agent->id])); ?>" class="px-2.5 py-1.5 rounded-lg text-xs font-semibold bg-indigo-50 hover:bg-indigo-100 text-indigo-700 transition inline-flex items-center gap-1" title="View Assigned Patients">
                                <i class="fas fa-list-check"></i>
                                <span class="hidden sm:inline">Tasks</span>
                            </a>

                            <!-- Toggle Status Form -->
                            <form action="<?php echo e(route('admin.agents.toggle_status', $agent->id)); ?>" method="POST" class="inline">
                                <?php echo csrf_field(); ?>
                                <button type="submit" class="px-2.5 py-1.5 rounded-lg text-xs font-semibold <?php echo e($agent->status === 'active' ? 'bg-amber-50 text-amber-700 hover:bg-amber-100' : 'bg-emerald-50 text-emerald-700 hover:bg-emerald-100'); ?> transition">
                                    <?php echo e($agent->status === 'active' ? 'Deactivate' : 'Activate'); ?>

                                </button>
                            </form>

                            <!-- Delete Form -->
                            <form action="<?php echo e(route('admin.agents.destroy', $agent->id)); ?>" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to remove this agent? Existing assignments will be unlinked.');">
                                <?php echo csrf_field(); ?>
...
<div id="addAgentModal" class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 hidden">
    <div class="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-5">
        ...
        <form action="<?php echo e(route('admin.agents.store')); ?>" method="POST" class="space-y-4 text-xs">
            <?php echo csrf_field(); ?>
            <div>
...
<?php echo $__env->make('admin.layout.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\laravel\outsourcelab\resources\views/admin/agents/index.blade.php ENDPATH**/ ?>