<?php $__env->startSection('title', 'Doorstep Money Collections'); ?>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div class="bg-gradient-to-br from-emerald-600 to-teal-700 rounded-2xl p-5 text-white shadow-lg shadow-teal-700/20">
            ...
            <div class="flex items-baseline gap-2 mt-1">
                <span class="text-3xl font-black text-white">₹<?php echo e(number_format($totalCashInHand)); ?></span>
        ...
        <div class="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm">
            ...
            <div class="flex items-baseline gap-2 mt-1">
                <span class="text-3xl font-black text-slate-900">₹<?php echo e(number_format($totalPendingCash)); ?></span>
            </div>
        ...
        <div class="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm">
            ...
            <div class="flex items-baseline gap-2 mt-1">
                <span class="text-3xl font-black text-slate-900"><?php echo e($totalOnlinePaid); ?></span>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200/80 overflow-hidden">
        <div class="p-4 sm:p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
            <h2 class="text-sm sm:text-base font-extrabold text-slate-800 flex items-center gap-2">
                ...
                <span>Client Payment Records (<?php echo e($bookings->count()); ?>)</span>
            </h2>
        </div>
                ...
                <div class="p-4 sm:p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 hover:bg-slate-50/40 transition">
                    ...
                    <div class="space-y-1">
                        <div class="flex items-center gap-2">
                            ...
                            <?php if($booking->familyMember): ?>
                                <span class="text-[11px] text-slate-500 font-semibold">(<?php echo e($booking->familyMember->name); ?>)</span>
                            <?php endif; ?>
                        </div>
                        <div class="text-xs text-slate-500 flex flex-wrap items-center gap-3">
                            <?php if($patientPhone): ?>
                                <span><i class="fas fa-phone text-slate-400 mr-1"></i> <?php echo e($patientPhone); ?></span>
                            <?php endif; ?>
                            <span><i class="far fa-calendar mr-1"></i> <?php echo e($booking->booking_date ? $booking->booking_date->format('M d, Y') : 'Today'); ?></span>
                            <span><i class="fas fa-wallet text-slate-400 mr-1"></i> Method: <strong><?php echo e($booking->payment_method); ?></strong></span>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-wrap items-center gap-3 self-stretch sm:self-auto justify-between sm:justify-end border-t sm:border-t-0 pt-3 sm:pt-0 border-slate-100">
                        <div class="text-left sm:text-right">
                            ...
                            <span class="text-lg font-black text-slate-900">₹<?php echo e(number_format($booking->amount)); ?></span>
                        </div>
                            ...
                            <?php elseif($isCollectedByMe): ?>
                                <div class="text-right">
                                    ...
                                    <span class="text-[10px] text-slate-500 block mt-1 font-medium">
                                        <?php echo e($booking->money_collected_at ? $booking->money_collected_at->format('M d, h:i A') : 'Recorded'); ?> (<?php echo e($booking->money_payment_mode ?? 'Cash'); ?>)
                                    </span>
                                </div>
                            <?php else: ?>
                                <div class="flex items-center gap-2">
                                    <form action="<?php echo e(route('agent.collect_money', $booking->id)); ?>" method="POST" class="flex items-center gap-1.5">
                                        <?php echo csrf_field(); ?>
                                        <select name="payment_mode" class="text-xs font-semibold px-2 py-1.5 border border-slate-300 rounded-lg bg-white">
                                            <option value="Cash">Cash</option>
                                            <option value="UPI / QR">UPI / QR</option>
                                        </select>
                                        <button type="submit" onclick="return confirm('Confirm receipt of ₹<?php echo e($booking->amount); ?> from this patient?');" class="inline-flex items-center gap-1 px-3 py-1.5 rounded-xl bg-amber-500 hover:bg-amber-600 text-slate-950 text-xs font-black shadow-sm transition">
                                            <i class="fas fa-hand-holding-usd"></i> Collect ₹<?php echo e($booking->amount); ?>
...
<?php echo $__env->make('agent.layout.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\lab\lab\resources\views/agent/collections.blade.php ENDPATH**/ ?>