...
@section('title', isset($coupon) ? 'Edit Coupon: ' . $coupon->code : 'Create Coupon')
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-6">
        <div class="flex items-center space-x-3">
            <a href="{{ route('admin.coupons.index') }}" class="w-9 h-9 rounded-xl bg-white dark:bg-white/[0.05] border border-slate-200 dark:border-white/[0.08] text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white flex items-center justify-center transition-all shadow-sm">
                <i class="fas fa-arrow-left text-xs"></i>
            </a>
            <div>
                <h1 class="text-xl sm:text-2xl font-black text-slate-900 dark:text-white">
                    {{ isset($coupon) ? 'Edit Coupon: ' . $coupon->code : 'Create New Coupon' }}
                </h1>
            ...
            <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <div class="flex gap-2">
                                <input type="text" name="code" id="couponCodeInput" value="{{ old('code', $coupon->code ?? '') }}" required placeholder="e.g. WELCOME15, SAVE20"
                        ...
                        <div>
                            ...
                            <input type="text" name="title" value="{{ old('title', $coupon->title ?? '') }}" required placeholder="e.g. First Time Login Welcome Discount"
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <textarea name="description" rows="2" placeholder="e.g. Enjoy 15% discount on all health packages & lab tests for your first diagnostic order."
                                class="w-full px-4 py-2.5 rounded-xl bg-slate-50 dark:bg-white/[0.04] border border-slate-200 dark:border-white/[0.08] text-xs text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50">{{ old('description', $coupon->description ?? '') }}</textarea>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        ...
                        <div>
                            ...
                            <select name="coupon_type" id="couponTypeSelect" onchange="onTypeChange()" class="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 dark:bg-white/[0.04] border border-slate-200 dark:border-white/[0.08] text-xs text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 font-medium">
                                <option value="welcome" {{ old('coupon_type', $coupon->coupon_type ?? 'welcome') === 'welcome' ? 'selected' : '' }}>🎁 First Login Welcome (Auto assigned to users)</option>
                                <option value="banner" {{ old('coupon_type', $coupon->coupon_type ?? '') === 'banner' ? 'selected' : '' }}>📢 Banner Offer (Min Order Threshold)</option>
                                <option value="general" {{ old('coupon_type', $coupon->coupon_type ?? '') === 'general' ? 'selected' : '' }}>🏷️ Standard Coupon</option>
                            </select>
                        ...
                        <div>
                            ...
                            <select name="discount_type" class="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 dark:bg-white/[0.04] border border-slate-200 dark:border-white/[0.08] text-xs text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 font-medium">
                                <option value="percentage" {{ old('discount_type', $coupon->discount_type ?? 'percentage') === 'percentage' ? 'selected' : '' }}>Percentage (%) Discount</option>
                                <option value="fixed" {{ old('discount_type', $coupon->discount_type ?? '') === 'fixed' ? 'selected' : '' }}>Fixed Amount (₹) Flat Discount</option>
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input type="number" step="0.01" name="discount_value" value="{{ old('discount_value', $coupon->discount_value ?? '15') }}" required min="0.01" placeholder="e.g. 15 or 200"
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="number" step="0.01" name="min_order_amount" id="minOrderInput" value="{{ old('min_order_amount', $coupon->min_order_amount ?? '0') }}" min="0" placeholder="0"
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <span class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 font-bold text-sm">₹</span>
                                <input type="number" step="0.01" name="max_discount_amount" value="{{ old('max_discount_amount', $coupon->max_discount_amount ?? '') }}" min="0" placeholder="e.g. 500"
                                    class="w-full pl-8 pr-4 py-2.5 rounded-xl bg-slate-50 dark:bg-white/[0.04] border border-slate-200 dark:border-white/[0.08] text-sm font-bold text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50">
                            </div>
                ...
                <div class="p-5 rounded-2xl bg-amber-500/5 border border-amber-500/20">
                    ...
                    <div class="space-y-4">
                        <div class="flex items-center">
                            <input type="checkbox" name="is_banner" id="isBannerCheckbox" value="1" {{ old('is_banner', $coupon->is_banner ?? false) ? 'checked' : '' }}
                        ...
                        <div>
                            ...
                            <input type="text" name="banner_text" id="bannerTextInput" 
                                data-is-edit="{{ isset($coupon) && !empty($coupon->banner_text) ? '1' : '0' }}"
                                value="{{ old('banner_text', $coupon->banner_text ?? '') }}" placeholder="e.g. Flat 20% OFF on all lab tests for bookings above ₹999! Use Code: WELLCARE20"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="date" name="valid_from" value="{{ old('valid_from', isset($coupon->valid_from) ? $coupon->valid_from->format('Y-m-d') : '') }}"
                    ...
                    <div>
                        ...
                        <input type="date" name="valid_until" value="{{ old('valid_until', isset($coupon->valid_until) ? $coupon->valid_until->format('Y-m-d') : '') }}"
                            class="w-full px-4 py-2.5 rounded-xl bg-slate-50 dark:bg-white/[0.04] border border-slate-200 dark:border-white/[0.08] text-xs text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50">
                    </div>

                    <div class="flex items-center pt-6">
                        <input type="checkbox" name="is_active" id="isActiveCheckbox" value="1" {{ old('is_active', $coupon->is_active ?? true) ? 'checked' : '' }}
                ...
                <div class="flex items-center justify-end gap-3 pt-6 border-t border-slate-100 dark:border-white/[0.06]">
                    <a href="{{ route('admin.coupons.index') }}" class="px-5 py-2.5 rounded-xl border border-slate-200 dark:border-white/[0.08] text-xs font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-50 transition">
                        Cancel
                    </a>
                    <button type="submit" class="px-6 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold shadow-md shadow-indigo-500/20 transition-all">
                        {{ isset($coupon) ? 'Update Coupon' : 'Save & Publish Coupon' }}
                    </button>
                </div>
            </div>
        </form>
