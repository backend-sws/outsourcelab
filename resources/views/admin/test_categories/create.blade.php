...
<div class="px-6 py-6 sm:px-8 max-w-4xl mx-auto">
    ...
    <div class="flex items-center space-x-4 mb-8">
        <a href="{{ route('admin.departments.index') }}" class="p-2 rounded-xl bg-slate-800/50 text-slate-400 hover:text-white hover:bg-slate-700/50 transition-all border border-slate-700/50">
            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
            </svg>
        </a>
        <div>
            <h1 class="text-3xl font-bold text-white tracking-tight">Create Test</h1>
            <p class="text-slate-400 text-sm mt-1">Add a new health test service Test parameters.</p>
        </div>
    </div>
    ...
    <div class="bg-slate-800/50 backdrop-blur-xl rounded-2xl border border-slate-700/50 shadow-xl overflow-hidden relative">
        ...
        <form action="{{ route('admin.departments.store') }}" method="POST" class="p-8">
            @csrf

            <!-- Name Field -->
            <div class="mb-6">
                <label for="name" class="block text-sm font-medium text-slate-300 mb-2">Test Name</label>
            ...
            <div class="pt-4 flex justify-end">
                <a href="{{ route('admin.departments.index') }}" class="px-6 py-3 mr-3 font-medium text-slate-300 transition-colors hover:text-white">
                    Cancel
                </a>
