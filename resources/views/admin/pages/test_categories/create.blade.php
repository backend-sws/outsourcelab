...
<div class="max-w-xl mx-auto space-y-6">
    ...
    <div class="flex items-center space-x-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Create Department</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Add a new diagnostic department (e.g. Biochemistry, Haematology). Individual test parameters are set inside each Test.</p>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-[#12142d] rounded-2xl border border-slate-200 dark:border-white/[0.08] shadow-sm p-6 sm:p-8">
        <form action="{{ route('admin.departments.store') }}" method="POST" class="space-y-6">
            ...
            <div class="p-3.5 rounded-xl bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800/40 text-xs text-blue-700 dark:text-blue-300 flex items-start gap-2">
                ...
                <span>
                    <strong>Tip:</strong> After creating a department, add Tests inside it (Admin → Tests → Add New Test) and set each test's parameters (e.g. Hemoglobin, RBC Count, WBC).
                </span>
            </div>
