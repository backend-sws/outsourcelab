...
<div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
    ...
    <div class="flex items-center gap-2">
        <a href="{{ route('admin.coupons.create') }}" class="px-4 py-2.5 rounded-xl text-xs font-bold bg-indigo-600 hover:bg-indigo-700 text-white transition-all flex items-center gap-2 shadow-sm shadow-indigo-500/20">
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
    ...
    <div class="p-5 rounded-2xl bg-white dark:bg-[#12142d] border border-slate-200 dark:border-white/[0.06] shadow-sm flex items-center justify-between">
        <div>
            ...
            <h3 class="text-2xl font-black text-slate-900 dark:text-white mt-1.5">{{ number_format($totalCoupons) }}</h3>
    ...
    <div class="p-5 rounded-2xl bg-white dark:bg-[#12142d] border border-slate-200 dark:border-white/[0.06] shadow-sm flex items-center justify-between">
        <div>
            ...
            <h3 class="text-2xl font-black text-slate-900 dark:text-white mt-1.5">{{ number_format($welcomeCoupons) }} Active</h3>
    ...
    <div class="p-5 rounded-2xl bg-white dark:bg-[#12142d] border border-slate-200 dark:border-white/[0.06] shadow-sm flex items-center justify-between">
        <div>
            ...
            <h3 class="text-2xl font-black text-slate-900 dark:text-white mt-1.5">{{ number_format($bannerCoupons) }} Live</h3>
    ...
    <div class="p-5 rounded-2xl bg-white dark:bg-[#12142d] border border-slate-200 dark:border-white/[0.06] shadow-sm flex items-center justify-between">
        <div>
            ...
            <h3 class="text-2xl font-black text-slate-900 dark:text-white mt-1.5">{{ number_format($totalUsedCoupons) }}</h3>
...
<div class="p-4 rounded-2xl bg-white dark:bg-[#12142d] border border-slate-200 dark:border-white/[0.06] shadow-sm">
    <form method="GET" action="{{ route('admin.coupons.index') }}" class="flex flex-col lg:flex-row gap-3 items-stretch lg:items-center justify-between">
        <div class="flex-1 flex flex-col sm:flex-row gap-3">
            <!-- Search -->
            <div class="relative flex-1">
                <i class="fas fa-search absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 text-xs"></i>
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by coupon code or title..." 
            ...
            <div class="sm:w-52">
                <select name="type" onchange="this.form.submit()" class="w-full px-3.5 py-2.5 bg-slate-50 dark:bg-white/[0.04] border border-slate-200 dark:border-white/[0.08] rounded-xl text-xs text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/50">
                    ...
                    <option value="welcome" {{ request('type') === 'welcome' ? 'selected' : '' }}>First Login Welcome</option>
                    <option value="banner" {{ request('type') === 'banner' ? 'selected' : '' }}>Banner (Min Order Amount)</option>
                    <option value="general" {{ request('type') === 'general' ? 'selected' : '' }}>General Promotional</option>
                </select>
            </div>
        ...
        <div class="flex items-center gap-2">
            ...
            @if(request()->hasAny(['search', 'type', 'status']))
                <a href="{{ route('admin.coupons.index') }}" class="px-3.5 py-2.5 rounded-xl bg-slate-100 dark:bg-white/[0.05] text-slate-600 dark:text-slate-300 hover:text-slate-900 text-xs font-semibold transition-colors">
                    Reset
                </a>
            @endif
        </div>
                    ...
                    <tr class="hover:bg-slate-50/60 dark:hover:bg-white/[0.02] transition-colors group">
                        ...
                        <td class="px-6 py-4">
                            ...
                            <div class="mt-1 flex items-center gap-1.5 flex-wrap">
                                ...
                                @elseif($coupon->coupon_type === 'banner')
                                    <span class="inline-flex items-center text-[10px] font-bold px-2 py-0.5 rounded bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-500/30">
                                        <i class="fas fa-bullhorn mr-1 text-[9px]"></i> Banner Offer (Spend > ₹{{ number_format($coupon->min_order_amount) }})
                                    </span>
                                @else
                            ...
                            <div class="font-black text-base text-slate-800 dark:text-slate-200">
                                @if($coupon->discount_type === 'percentage')
                                    ...
                                    @if($coupon->max_discount_amount)
                                        <span class="text-[10px] text-slate-400 font-normal block">Up to ₹{{ number_format($coupon->max_discount_amount) }}</span>
                                    @endif
                                @else
                                    ₹{{ number_format($coupon->discount_value, 2) }} FLAT OFF
                                @endif
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4 text-center">
                            @if($coupon->is_banner)
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-amber-100 text-amber-800 dark:bg-amber-500/20 dark:text-amber-300 border border-amber-200 dark:border-amber-500/30" title="{{ $coupon->banner_text ?: 'Live on website header' }}">
                        ...
                        <!-- Status -->
                        <td class="px-6 py-4 text-center">
                            <form action="{{ route('admin.coupons.toggle', $coupon->id) }}" method="POST" class="inline-block">
                                @csrf
                                <button type="submit" title="Click to toggle status" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-bold transition-transform active:scale-95 {{ $coupon->is_active ? 'bg-green-100 text-green-800 dark:bg-green-500/20 dark:text-green-300' : 'bg-gray-100 text-gray-600 dark:bg-white/[0.06] dark:text-slate-400' }}">
                                    <span class="w-1.5 h-1.5 rounded-full {{ $coupon->is_active ? 'bg-green-500' : 'bg-gray-400' }} mr-1.5"></span>
                        ...
                        <td class="px-6 py-4 text-right space-x-1.5">
                            <a href="{{ route('admin.coupons.edit', $coupon->id) }}" class="inline-flex items-center p-2 rounded-lg text-blue-600 hover:text-blue-900 bg-blue-50 dark:bg-blue-500/10 hover:bg-blue-100 transition-colors">
                                <i class="fas fa-edit text-xs"></i>
                            </a>

                            <form action="{{ route('admin.coupons.destroy', $coupon->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure you want to delete coupon \'{{ $coupon->code }}\'?');">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="8" class="px-6 py-12 text-center">
                            ...
                            <a href="{{ route('admin.coupons.create') }}" class="mt-4 inline-flex items-center px-4 py-2 rounded-xl bg-indigo-600 text-white text-xs font-semibold hover:bg-indigo-700 transition shadow-sm">
