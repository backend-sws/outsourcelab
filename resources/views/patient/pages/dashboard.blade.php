        ...
        <div class="w-full md:w-2/3 lg:w-3/4 space-y-6">
            ...
            <div class="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-6 pb-6 border-b border-gray-100">
                        ...
                        <div>
                            <div class="flex items-center gap-2 flex-wrap">
                                ...
                                @if($profile->isVipMember())
                                    ...
                                    <a href="{{ route('patient.membership') }}" class="inline-flex items-center gap-1.5 bg-gradient-to-r from-amber-500 to-amber-600 text-white text-[11px] font-black px-3 py-0.5 rounded-full shadow-sm hover:from-amber-600 hover:to-amber-700 transition">
                                        <i class="fas fa-crown text-amber-200 animate-pulse"></i> {{ $activeVip->plan_name_snapshot }} ({{ $activeVip->discount_percentage }}% OFF)
                                    </a>
                                @else
                                    <a href="{{ route('patient.membership') }}" class="inline-flex items-center gap-1.5 bg-amber-50 border border-amber-300 text-amber-800 text-[11px] font-bold px-2.5 py-0.5 rounded-full hover:bg-amber-100 transition">
                                        <i class="fas fa-crown text-amber-500"></i> Join VIP Club & Save
                                    </a>
                                @endif

                                <a href="{{ route('patient.rewards') }}" class="inline-flex items-center gap-1.5 bg-amber-50 border border-amber-200 text-amber-900 text-[11px] font-black px-2.5 py-0.5 rounded-full hover:bg-amber-100 transition shadow-xs">
                                    <i class="fas fa-coins text-amber-500"></i> {{ number_format($profile->reward_coins ?? 0) }} Health Coins
                                </a>
                            </div>
                    ...
                    <a href="#profile-form" class="inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-brand-dark hover:bg-brand-primary text-white text-xs font-extrabold rounded-xl shadow-sm transition">
                        <i class="far fa-edit"></i> Edit Form Below
                    </a>
                </div>

                <!-- Quick Stats -->
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-5">
                    <a href="{{ route('patient.bookings') }}" class="p-3 bg-gray-50/80 hover:bg-teal-50/60 rounded-xl border border-gray-100 hover:border-teal-200 transition group">
                        ...
                        <div class="flex items-center justify-between mt-1">
                            <span class="text-xl font-black text-brand-dark">{{ $profile->bookings->count() }}</span>
                            <i class="fas fa-calendar-check text-gray-300 group-hover:text-brand-primary transition text-sm"></i>
                        </div>
                    </a>
                    <a href="{{ route('patient.reports') }}" class="p-3 bg-gray-50/80 hover:bg-teal-50/60 rounded-xl border border-gray-100 hover:border-teal-200 transition group">
                        <span class="block text-[11px] font-bold text-gray-400 uppercase tracking-wider group-hover:text-brand-dark">Lab Reports</span>
                        <div class="flex items-center justify-between mt-1">
                            <span class="text-xl font-black text-brand-dark">{{ $profile->bookings->whereIn('status', ['Completed', 'Report Ready'])->count() }}</span>
                            <i class="fas fa-file-medical-alt text-gray-300 group-hover:text-brand-primary transition text-sm"></i>
                        </div>
                    </a>
                    <a href="{{ route('patient.family_members') }}" class="p-3 bg-gray-50/80 hover:bg-teal-50/60 rounded-xl border border-gray-100 hover:border-teal-200 transition group">
                        ...
                        <div class="flex items-center justify-between mt-1">
                            <span class="text-xl font-black text-brand-dark">{{ $profile->familyMembers->count() }}</span>
                            <i class="fas fa-users text-gray-300 group-hover:text-brand-primary transition text-sm"></i>
                        </div>
                    </a>
                    <a href="{{ route('patient.address_book') }}" class="p-3 bg-gray-50/80 hover:bg-teal-50/60 rounded-xl border border-gray-100 hover:border-teal-200 transition group">
                        <span class="block text-[11px] font-bold text-gray-400 uppercase tracking-wider group-hover:text-brand-dark">Saved Addresses</span>
                        <div class="flex items-center justify-between mt-1">
                            <span class="text-xl font-black text-brand-dark">{{ $profile->addresses->count() }}</span>
                            <i class="fas fa-map-marker-alt text-gray-300 group-hover:text-brand-primary transition text-sm"></i>
                        </div>
                    </a>
                </div>
                ...
                @if($profile->isVipMember())
                    @php $activeVip = $profile->activeMembership(); @endphp
                    <div class="mt-4 p-3.5 rounded-xl bg-gradient-to-r from-amber-500/10 via-amber-400/5 to-transparent border border-amber-200/80 flex items-center justify-between gap-4">
                        <div class="flex items-center gap-3">
                            <div class="w-8 h-8 rounded-lg bg-amber-500 text-white flex items-center justify-center font-bold text-sm shadow-sm">
                                <i class="fas fa-crown"></i>
                            </div>
                            <div>
                                <p class="text-xs font-black text-brand-dark">Active VIP Privileges: <span class="text-amber-700">{{ $activeVip->plan_name_snapshot }}</span></p>
                                <p class="text-[11px] text-gray-500 font-medium">Flat {{ $activeVip->discount_percentage }}% OFF applied on all lab tests • Free home sample collection</p>
                            </div>
                        </div>
                        <a href="{{ route('patient.membership') }}" class="px-3 py-1.5 rounded-lg bg-amber-500 hover:bg-amber-600 text-white text-xs font-extrabold transition shadow-xs flex-shrink-0">
                            View Card
                        </a>
                    </div>
                @endif
