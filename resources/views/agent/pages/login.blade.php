            ...
            <form action="{{ route('agent.login.submit') }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="password" name="password" id="agentPassword" required value="password123" class="w-full pl-10 pr-10 py-3 bg-slate-800/80 border border-white/10 rounded-xl text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-teal-500/60 focus:border-teal-500 transition" placeholder="••••••••">
                        <button type="button" onclick="const p = document.getElementById('agentPassword'); const isP = p.type === 'password'; p.type = isP ? 'text' : 'password'; this.querySelector('i').className = isP ? 'far fa-eye-slash text-xs text-slate-400' : 'far fa-eye text-xs text-slate-500';" class="absolute right-3.5 top-1/2 -translate-y-1/2 text-slate-500 hover:text-slate-300 transition">
